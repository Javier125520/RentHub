...
public class InmuebleServicioDAO {
    ...
    private static final String SELECT_BY_INMUEBLE =
            "SELECT id_servicio, precio_adicional, estado_servicio " +
                    "FROM inmueble_servicio WHERE id_inmueble = ?";
    ...
    public List<InmuebleServicio> findServiciosByInmueble(Inmueble inmueble) throws SQLException {

        List<InmuebleServicio> lista = new ArrayList<>();

        try (PreparedStatement ps = conn.prepareStatement(SELECT_SERVIOS_BY_INMUEBLE)) {
            ps.setInt(1, inmueble.getIdInmueble());
